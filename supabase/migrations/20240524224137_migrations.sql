...
CREATE OR REPLACE FUNCTION "public"."app_admin_get_all_organizations"(
    ...
    "credits" bigint
  ) LANGUAGE "plpgsql" AS $$ BEGIN IF CURRENT_ROLE <> 'service_role' THEN RAISE EXCEPTION 'Only service_role can execute this function';
END IF;
...
ALTER FUNCTION "public"."app_admin_get_all_organizations"(
...
CREATE OR REPLACE FUNCTION "public"."app_admin_get_all_organizations_count"(
    "search_query" character varying DEFAULT ''::character varying
  ) RETURNS bigint LANGUAGE "plpgsql" AS $$ BEGIN IF CURRENT_ROLE NOT IN (
    'service_role',
    'supabase_admin',
    'dashboard_user',
    'postgres'
...
ALTER FUNCTION "public"."app_admin_get_all_users"(
  "search_query" character varying,
  "page" integer,
  "page_size" integer
) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."app_admin_get_organizations_created_per_month"() RETURNS TABLE(
    "month" "date",
...
CREATE OR REPLACE FUNCTION "public"."check_if_user_is_app_admin"("user_id" "uuid") RETURNS boolean LANGUAGE "plpgsql" SECURITY DEFINER AS $$ -- Begin the function
...
CREATE OR REPLACE FUNCTION "public"."get_all_app_admins"() RETURNS TABLE("user_id" "uuid") LANGUAGE "plpgsql" SECURITY DEFINER AS $$ BEGIN RETURN QUERY
...
CREATE OR REPLACE FUNCTION "public"."get_invited_organizations_for_user_v2"("user_id" "uuid", "user_email" character varying) RETURNS TABLE("organization_id" "uuid") LANGUAGE "plpgsql" SECURITY DEFINER AS $$ BEGIN IF (user_id IS NULL)
  AND (
    user_email IS NULL
    OR user_email = ''
  ) THEN RETURN QUERY
...
CREATE OR REPLACE FUNCTION "public"."get_organization_admin_ids"("organization_id" "uuid") RETURNS TABLE("member_id" "uuid") LANGUAGE "plpgsql" SECURITY DEFINER AS $_$ BEGIN -- This function returns the member_id column for all rows in the organization_members table
  RETURN QUERY
...
CREATE OR REPLACE FUNCTION "public"."get_organization_id_for_project_id"("project_id" "uuid") RETURNS "uuid" LANGUAGE "plpgsql" SECURITY DEFINER AS $$
DECLARE org_id UUID;
BEGIN
...
CREATE OR REPLACE FUNCTION "public"."get_organization_member_ids"("organization_id" "uuid") RETURNS TABLE("member_id" "uuid") LANGUAGE "plpgsql" SECURITY DEFINER AS $_$ BEGIN -- This function returns the member_id column for all rows in the organization_members table
  RETURN QUERY
SELECT organization_members.member_id
FROM organization_members
WHERE organization_members.organization_id = $1;
...
CREATE OR REPLACE FUNCTION "public"."get_organizations_for_user"("user_id" "uuid") RETURNS TABLE("organization_id" "uuid") LANGUAGE "plpgsql" SECURITY DEFINER AS $$ BEGIN RETURN QUERY
SELECT o.id AS organization_id
FROM organizations o
...
CREATE OR REPLACE FUNCTION "public"."handle_add_organization_member_after_invitation_accepted"() RETURNS "trigger" LANGUAGE "plpgsql" SECURITY DEFINER AS $$BEGIN
...
CREATE OR REPLACE FUNCTION "public"."handle_organization_created"() RETURNS "trigger" LANGUAGE "plpgsql" SECURITY DEFINER AS $$ BEGIN
...
CREATE OR REPLACE FUNCTION "public"."handle_organization_created_add_credits"() RETURNS "trigger" LANGUAGE "plpgsql" SECURITY DEFINER AS $$ BEGIN
...
CREATE OR REPLACE FUNCTION "public"."increment_credits"("org_id" "uuid", "amount" integer) RETURNS "void" LANGUAGE "plpgsql" SECURITY DEFINER AS $$ BEGIN -- Decrement the credits column by the specified amount
UPDATE organization_credits
SET credits = credits + amount
WHERE organization_id = org_id;
END;
...
ALTER TABLE "public"."internal_blog_post_tags"
ALTER COLUMN "id"
ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME "public"."internal_blog_post_tags_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
  );
...
ALTER COLUMN "id"
ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."project_comments_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
  );
...
CREATE OR REPLACE TRIGGER "on_organization_created_credits"
AFTER
INSERT ON "public"."organizations" FOR EACH ROW EXECUTE FUNCTION "public"."handle_organization_created_add_credits"();
...
CREATE OR REPLACE TRIGGER "set_user_id_before_insert" BEFORE
INSERT ON "public"."chats" FOR EACH ROW EXECUTE FUNCTION "public"."set_default_user_id"();
...
CREATE POLICY "All authenticated members can insert" ON "public"."projects" FOR
...
CREATE POLICY "All organization members can read organizations v2" ON "public"."organizations" FOR
...
CREATE POLICY "All organization members of a project can make project comments" ON "public"."project_comments" FOR
INSERT TO "authenticated" WITH CHECK (
    (
      (
        SELECT "auth"."uid"() AS "uid"
      ) IN (
        SELECT "public"."get_organization_member_ids"(
            "public"."get_organization_id_for_project_id"("project_comments"."project_id")
          ) AS "get_organization_member_ids"
      )
    )
  );

CREATE POLICY "All organization members of a project can read project comments" ON "public"."project_comments" FOR
SELECT TO "authenticated" USING (
    (
      (
        SELECT "auth"."uid"() AS "uid"
...
CREATE POLICY "Allow  any to read admin blog author posts" ON "public"."internal_blog_author_posts" FOR
SELECT USING (TRUE);

CREATE POLICY "Allow anyone to read admin blog author profiles" ON "public"."internal_blog_author_profiles" FOR
SELECT USING (TRUE);

CREATE POLICY "Allow anyone to read admin blog post tags" ON "public"."internal_blog_post_tags" FOR
SELECT USING (TRUE);

CREATE POLICY "Allow anyone to read admin blog posts" ON "public"."internal_blog_posts" FOR
SELECT USING (TRUE);

CREATE POLICY "Allow anyone to read admin blog tag relationships" ON "public"."internal_blog_post_tags_relationship" FOR
SELECT USING (TRUE);

CREATE POLICY "Allow anyone to read changelog" ON "public"."internal_changelog" FOR
SELECT USING (TRUE);
...
CREATE POLICY "Changelog is visible to everyone" ON "public"."internal_changelog" FOR
SELECT USING (TRUE);

CREATE POLICY "Enable delete for team admins" ON "public"."projects" FOR DELETE USING (
  (
    (
      SELECT "auth"."uid"() AS "uid"
    ) IN (
      SELECT "public"."get_organization_admin_ids"("projects"."organization_id") AS "get_organization_admin_ids"
    )
  )
);

CREATE POLICY "Enable delete for users based on user_id" ON "public"."organization_members" FOR DELETE TO "authenticated" USING (
  (
    (
      SELECT "auth"."uid"() AS "uid"
    ) IN (
...
CREATE POLICY "Enable insert for authenticated users only" ON "public"."user_onboarding" FOR
INSERT TO "authenticated" WITH CHECK (
    (
      (
        SELECT "auth"."uid"() AS "uid"
...
CREATE POLICY "Enable read access for all organization members" ON "public"."projects" FOR
SELECT TO "authenticated" USING (
    (
      (
        SELECT "auth"."uid"() AS "uid"
      ) IN (
        SELECT "public"."get_organization_member_ids"("projects"."organization_id") AS "get_organization_member_ids"
      )
    )
  );

CREATE POLICY "Enable read access for all users" ON "public"."organization_credits" FOR
SELECT TO "authenticated" USING (
    (
      (
        SELECT "auth"."uid"() AS "uid"
      ) IN (
        SELECT "public"."get_organization_member_ids"("organization_credits"."organization_id") AS "get_organization_member_ids"
      )
    )
  );

CREATE POLICY "Enable update for org members" ON "public"."projects" FOR
UPDATE USING (
    (
      (
        SELECT "auth"."uid"() AS "uid"
...
CREATE POLICY "Everyone organization member can view the subscription on  orga" ON "public"."subscriptions" FOR
SELECT TO "authenticated" USING (
    (
      (
        SELECT "auth"."uid"() AS "uid"
...
CREATE POLICY "Feedback Comments Create Policy" ON "public"."internal_feedback_comments" FOR
INSERT TO "authenticated" WITH CHECK (TRUE);

CREATE POLICY "Feedback Comments Owner Delete Policy" ON "public"."internal_feedback_comments" FOR DELETE TO "authenticated" USING (
  (
    "user_id" = (
      SELECT "auth"."uid"() AS "uid"
    )
...
CREATE POLICY "Feedback Comments Owner Update Policy" ON "public"."internal_feedback_comments" FOR
UPDATE TO "authenticated" USING (
    (
      "user_id" = (
        SELECT "auth"."uid"() AS "uid"
...
CREATE POLICY "Feedback Comments View Policy" ON "public"."internal_feedback_comments" FOR
SELECT TO "authenticated" USING (TRUE);

CREATE POLICY "Feedback Threads Create Policy" ON "public"."internal_feedback_threads" FOR
INSERT TO "authenticated" WITH CHECK (TRUE);

CREATE POLICY "Feedback Threads Owner Delete Policy" ON "public"."internal_feedback_threads" FOR DELETE TO "authenticated" USING (
  (
    "user_id" = (
      SELECT "auth"."uid"() AS "uid"
    )
...
CREATE POLICY "Feedback Threads Owner Update Policy" ON "public"."internal_feedback_threads" FOR
UPDATE TO "authenticated" USING (
    (
      "user_id" = (
        SELECT "auth"."uid"() AS "uid"
...
CREATE POLICY "Feedback Threads Visibility Policy" ON "public"."internal_feedback_threads" FOR
SELECT USING (
    (
...
CREATE POLICY "Inviter can delete the invitation" ON "public"."organization_join_invitations" FOR DELETE TO "authenticated" USING (
  (
    (
      SELECT "auth"."uid"() AS "uid"
    ) = "inviter_user_id"
...
CREATE POLICY "Only organization owners/admins can update private organization" ON "public"."organizations_private_info" FOR
UPDATE TO "authenticated" USING (
    (
      (
        SELECT "auth"."uid"() AS "uid"
...
CREATE POLICY "Only organization owners/admins can view private organizations " ON "public"."organizations_private_info" FOR
SELECT TO "authenticated" USING (
    (
      (
        SELECT "auth"."uid"() AS "uid"
...
CREATE POLICY "Person who created the comment can delete it" ON "public"."project_comments" FOR DELETE TO "authenticated" USING (
  (
    (
      SELECT "auth"."uid"() AS "uid"
    ) = "user_id"
...
CREATE POLICY "Person who created the comment can update it" ON "public"."project_comments" FOR
UPDATE TO "authenticated" USING (
    (
      (
        SELECT "auth"."uid"() AS "uid"
...
CREATE POLICY "Temporary : Everyone can view" ON "public"."organization_members" FOR
SELECT TO "authenticated" USING (TRUE);
...
CREATE POLICY "Users can update their onboarding status" ON "public"."user_onboarding" FOR
UPDATE TO "authenticated" USING (
    (
      (
        SELECT "auth"."uid"() AS "uid"
...
CREATE POLICY "Users can view their onboarding status" ON "public"."user_onboarding" FOR
SELECT TO "authenticated" USING (
    (
      (
        SELECT "auth"."uid"() AS "uid"
...
FROM "postgres";
GRANT ALL ON FUNCTION "public"."app_admin_get_all_organizations"(
    "search_query" character varying,
    "page" integer,
    "page_size" integer
  ) TO "service_role";

REVOKE ALL ON FUNCTION "public"."app_admin_get_all_organizations_count"("search_query" character varying)
FROM PUBLIC;
...
FROM "postgres";
GRANT ALL ON FUNCTION "public"."app_admin_get_all_users"(
    "search_query" character varying,
    "page" integer,
    "page_size" integer
  ) TO "service_role";

REVOKE ALL ON FUNCTION "public"."app_admin_get_all_users_count"("search_query" character varying)
FROM PUBLIC;
...
REVOKE ALL ON FUNCTION "public"."check_if_user_is_app_admin"("user_id" "uuid")
FROM PUBLIC;
...
REVOKE ALL ON FUNCTION "public"."get_invited_organizations_for_user_v2"("user_id" "uuid", "user_email" character varying)
FROM PUBLIC;
...
REVOKE ALL ON FUNCTION "public"."get_organization_admin_ids"("organization_id" "uuid")
FROM PUBLIC;
...
REVOKE ALL ON FUNCTION "public"."get_organization_id_for_project_id"("project_id" "uuid")
FROM PUBLIC;
...
REVOKE ALL ON FUNCTION "public"."get_organization_member_ids"("organization_id" "uuid")
FROM PUBLIC;
...
REVOKE ALL ON FUNCTION "public"."get_organizations_for_user"("user_id" "uuid")
FROM PUBLIC;
...
REVOKE ALL ON FUNCTION "public"."increment_credits"("org_id" "uuid", "amount" integer)
FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."increment_credits"("org_id" "uuid", "amount" integer) TO "service_role";

REVOKE ALL ON FUNCTION "public"."make_user_app_admin"("user_id_arg" "uuid")
FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."make_user_app_admin"("user_id_arg" "uuid") TO "service_role";

REVOKE ALL ON FUNCTION "public"."remove_app_admin_privilege_for_user"("user_id_arg" "uuid")
FROM PUBLIC;
