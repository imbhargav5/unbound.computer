...
export const createInternalFeedbackAction = authActionClient
  ...
  .action(
    async ({ parsedInput: { title, content, type }, ctx: { userId } }) => {
      ...
      remoteCache.admin.feedback.threads.byId.updateTag({ id: data.id });
      if (data.is_publicly_visible) {
        remoteCache.public.feedback.list.updateTag();
        remoteCache.public.feedback.threads.byId.updateTag({ id: data.id });
      }
...
export async function getLoggedInUserFeedbackById(feedbackId: string) {
  ...
  const { data, error } = await supabaseClient
    ...
    .eq("id", feedbackId)
    .or(
      `user_id.eq.${userId},added_to_roadmap.eq.true,open_for_public_discussion.eq.true,is_publicly_visible.eq.true`
    )
    .single();
...
export async function subscribeToBoardAction(boardId: string) {
  ...
  remoteCache.admin.feedback.boards.updateTag();
}
...
export async function subscribeToThreadAction(threadId: string) {
  ...
  remoteCache.admin.feedback.list.updateTag();
  remoteCache.public.feedback.list.updateTag();
  remoteCache.admin.feedback.threads.byId.updateTag({ id: threadId });
  remoteCache.public.feedback.threads.byId.updateTag({ id: threadId });
  remoteCache.public.feedback.recent.updateTag();
}
