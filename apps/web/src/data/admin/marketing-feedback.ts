...
import {
  adminUpdateFeedbackIdCaches,
  adminUpdateRoadmapCache,
...
export async function adminUpdateInternalFeedbackType({
  ...
  adminUpdateFeedbackIdCaches(feedbackId);
  return data;
}
...
export async function updateFeedbackBoard({
  ...
  remoteCache.admin.feedback.boards.bySlug.updateTag({ slug: data.slug });
  return data;
}
...
export async function deleteFeedbackBoard(boardId: string) {
  ...
  if (boardData?.slug) {
    remoteCache.admin.feedback.boards.bySlug.updateTag({
      slug: boardData.slug,
    });
  } else {
    remoteCache.admin.feedback.boards.list.updateTag();
...
export const updateFeedbackThreadBoardAction = adminActionClient
  .inputSchema(updateFeedbackThreadBoardSchema)
  .action(async ({ parsedInput: { threadId, boardId } }) => {
    ...
    adminUpdateFeedbackIdCaches(threadId);
    return data;
  });
...
export async function cachedGetAllFeedbackThreads() {
  "use cache: remote";
  cacheLife("minutes");
  remoteCache.admin.feedback.list.cacheTag();
