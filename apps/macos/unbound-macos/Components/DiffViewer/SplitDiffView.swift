        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            ForEach(hunks) { hunk in
                ...
                ForEach(Array(buildSplitLines(from: hunk.lines).enumerated()), id: \.offset) { _, pair in
                    SplitDiffLineRow(leftLine: pair.left, rightLine: pair.right)
                }
...
struct SplitDiffLineRow: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            SplitDiffLineCell(line: leftLine, side: .left)
                .frame(maxWidth: .infinity)
            ...
            SplitDiffLineCell(line: rightLine, side: .right)
                .frame(maxWidth: .infinity)
        }
    ...
    var body: some View {
        HStack(spacing: 0) {
            if let line = line {
                ...
                Text(line.content)
                    .font(Typography.code)
                    .foregroundStyle(line.type.textColor(colors: colors))
                    .frame(maxWidth: .infinity, alignment: .leading)
