...
struct DiffViewer: View {
    ...
    var body: some View {
        let language = SyntaxHighlighter.languageIdentifier(forFilePath: diff.filePath)
        VStack(spacing: 0) {
            // Header
            DiffViewerHeader(
                diff: diff,
            ...
            if !isCollapsed {
                ...
                if diff.isBinary {
                    ...
                    ScrollView {
                        switch viewMode {
                        case .unified:
                            UnifiedDiffView(hunks: diff.hunks, language: language)
                        case .split:
                            SplitDiffView(hunks: diff.hunks, language: language)
                        }
                    }
                }
            }
