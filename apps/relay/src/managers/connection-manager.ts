...
class ConnectionManager {
  ...
  /**
   * Add a new pre-authenticated connection
   */
  addAuthenticatedConnection(
    ...
    log.debug(
      ...
      "Authenticated connection added"
    );
    return connection;
  }

  /**
   * Add a new connection (for testing only - production uses addAuthenticatedConnection)
   * @internal
   */
  addConnection(deviceId: string, ws: WebSocket): Connection {
    ...
    log.debug({ deviceId }, "Connection added (unauthenticated)");
    return connection;
  }
