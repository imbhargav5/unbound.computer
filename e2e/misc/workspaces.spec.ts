...
import { getDefaultWorkspaceInfoHelper, goToWorkspaceArea, matchPathAndExtractWorkspaceInfo } from 'e2e/_helpers/workspace.helper';
...
test.describe.serial("Workspace", () => {
  ...
  test.describe("Workspace invite", () => {
    ...
    test("invite existing user to a workspace", async ({ browser, page }) => {
      ...
      const user2JWTCookie = user2Cookies.find(cookie => cookie.name === 'sb-localhost-auth-token');
      if (!user2JWTCookie) {
        throw new Error("No user JWT cookie found");
      }
      const user2Details = await getUserDetailsFromAuthJson(user2JWTCookie.value);
      ...
      const membersTable = user2Page.locator('table[data-testid="members-table"]');
      await membersTable.waitFor();
      const memberRow = membersTable.locator(`tr[data-user-id="${user2Details.id}"]`);
    ...
    test("invite new user to a workspace", async ({ page, browser }) => {
      ...
      await onboardUserHelper({ page: inviteePage, name: `Invitee John ${inviteeIdentifier}` });

      const inviteeCookies = await inviteeContext.cookies();
      console.log(inviteeCookies);
      const inviteeJWTCookie = inviteeCookies.find(cookie => cookie.name === 'sb-localhost-auth-token');
      if (!inviteeJWTCookie) {
        throw new Error("No user JWT cookie found");
      }
      ...
      const membersTable = inviteePage.locator('table[data-testid="members-table"]');
      await membersTable.waitFor();
      const memberRow = membersTable.locator(`tr[data-user-id="${inviteeDetails.id}"]`);
      await expect(memberRow).toBeVisible();
