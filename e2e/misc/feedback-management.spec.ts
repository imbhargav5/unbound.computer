...
test.describe.serial('Admin Feedback Management', () => {
  ...
  test('User creates feedback', async ({ browser, page: userPage }) => {
    ...
    await giveFeedbackForm.getByTestId('feedback-title-input').fill(feedbackTitle);
  ...
  test('Admin can view and update feedback', async ({ browser, page }) => {
    ...
    const dropdownMenuTrigger = feedbackVisibility.getByTestId('feedback-actions-dropdown-button')
    ...
    const prioritySubMenu = adminPage.getByTestId('apply-priority-dropdown-menu');
    ...
    await typeSubMenu.getByRole('menuitem', { name: 'Feature Request' }).click();
  ...
  test('Admin can add a comment', async ({ browser }) => {
    ...
    const commentsList = await adminPage.getByTestId('admin-user-feedback-comments');
  ...
  test('Admin can filter feedback', async ({ browser }) => {
    ...
    await adminPage.getByTestId('status-filter-command-list').getByRole('option', { name: 'In Progress' }).click();
    ...
    await adminPage.getByTestId('priority-filter-command-list').getByRole('option', { name: 'High' }).click();
    // wait for the url to change but no need to match the url
    await adminPage.waitForLoadState('load')
    // Filter by type
    await adminPage.getByTestId('type-filter-button').click();
    await adminPage.getByTestId('type-filter-command-list').getByRole('option', { name: 'Feature Request' }).click();
  ...
  test('Admin can toggle feedback visibility', async ({ browser }) => {
    ...
    await expect(adminPage.getByTestId('feedback-visibility').getByText('Private')).toBeVisible();
    ...
    await expect(adminPage.getByTestId('feedback-visibility').getByText('Public')).toBeVisible();
    await adminContext.close();
  });
