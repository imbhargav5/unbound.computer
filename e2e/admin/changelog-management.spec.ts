...
test.describe.serial('Changelog Management', () => {
  ...
  test('Admin creates a new changelog', async ({ browser }) => {
    ...
    await adminPage.waitForURL(/\/en\/app_admin\/marketing\/changelog\/[a-zA-Z0-9-]+$/);
    ...
    await adminPage.getByRole('link', { name: 'Marketing Changelog List' }).click();
  ...
  test('Admin can edit the changelog', async ({ browser }) => {
    ...
    const submitButton = await adminPage.getByRole('button', { name: 'Update Changelog' });
  ...
  test('Admin can delete the changelog', async ({ browser }) => {
    ...
    const deleteButton = adminPage.getByRole('row', { name: changelogTitle }).getByTestId('delete-changelog-dialog-trigger');
