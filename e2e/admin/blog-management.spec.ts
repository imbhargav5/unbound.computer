...
test.describe.serial('Blog Post Management', () => {
  ...
  test('Admin creates a new blog post', async ({ browser }) => {
    ...
    await adminPage.waitForURL(/\/en\/app_admin\/marketing\/blog\/[a-zA-Z0-9-]+$/);
  ...
  test('Admin can delete the blog post', async ({ browser }) => {
    ...
    const deleteButton = adminPage.getByRole('row', { name: blogPostTitle }).getByTestId('delete-blog-post-dialog-trigger');
