...
export async function extractInfoFromWorkspaceDashboard({
  ...
  page: Page;
}): Promise<{ workspaceId: string; workspaceSlug: string; isSoloWorkspace: boolean }> {
  ...
  const workspaceMembershipType = await page.getByTestId('isSoloWorkspace').textContent();
