...
test.skip("Users can forget password and reset", async ({ page, browser }) => {
  async function getResetPasswordEmail(
    ...
    const messagesReceivedWithinTheLastMinute = messagesWithSubjectReset.filter(
  ...
  const resetPasswordForm = newPage.locator('data-testid="password-form"');
  ...
  await newPage.goto("/logout");
  await newPage.goto("/login");
  await newPage.fill('input[data-strategy="email-password"]', email);
  await newPage.fill('input[name="password"]', "newpassword");
