...
test.describe.serial('Users can submit and view submitted feedback', () => {
  ...
  test('User can open feedback dialog and submit feedback', async ({ page }) => {
    ...
    const titleInput = await giveFeedbackForm.getByTestId('feedback-title-input');
    const contentInput = await giveFeedbackForm.getByTestId('feedback-content-input');
  ...
  test('Created feedback is visible on the feedback list page', async ({ page }) => {
    ...
    await expect(page.getByText('This is a test feedback content.')).toBeVisible();
  });
  ...
  test('User can add a comment to feedback and view it', async ({ page }) => {
    ...
    const commentsList = await page.getByTestId('logged-in-user-feedback-comments');
