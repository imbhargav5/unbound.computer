...
export const CreateTagButton: React.FC = () => {
  ...
  const createTagMutation = useAction(createTagAction, {
    onExecute: () => {
      toastRef.current = toast.loading('Creating tag...', { description: 'Please wait while we create the tag.' });
    },
    onSuccess: ({ data }) => {
    ...
    onError: ({ error }) => {
      toast.error(`Failed to create tag: ${error.serverError || 'Unknown error'}`, { id: toastRef.current });
      toastRef.current = undefined;
    },
  });
  ...
  return (
    <Button onClick={handleCreateTag} disabled={createTagMutation.status === 'executing'}>
