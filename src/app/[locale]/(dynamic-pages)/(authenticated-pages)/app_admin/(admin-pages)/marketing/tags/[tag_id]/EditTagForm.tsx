...
export const EditTagForm: React.FC<EditTagFormProps> = ({ tag }) => {
  ...
  const { register, handleSubmit, formState: { errors }, watch, setValue } = useForm<z.infer<typeof editTagSchema>>({
  ...
  const updateTagMutation = useAction(updateTagAction, {
    onExecute: () => {
      toastRef.current = toast.loading('Updating tag...', { description: 'Please wait while we update the tag.' });
    },
    onSuccess: () => {
    ...
    onError: ({ error }) => {
      toast.error(`Failed to update tag: ${error.serverError || 'Unknown error'}`, { id: toastRef.current });
      toastRef.current = undefined;
    },
  });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <Button type="submit" disabled={updateTagMutation.status === 'executing'}>
        {updateTagMutation.status === 'executing' ? 'Updating...' : 'Update Tag'}
      </Button>
    </form>
  );
};
