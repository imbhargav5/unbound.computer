...
export function DeleteAuthorProfileDialog({ authorId, authorName }: DeleteAuthorProfileDialogProps) {
  ...
  const deleteProfileMutation = useAction(deleteAuthorProfileAction, {
    onExecute: () => {
      toastRef.current = toast.loading('Deleting profile...', { description: 'Please wait while we delete the profile.' });
    },
    onSuccess: () => {
    ...
    onError: ({ error }) => {
      toast.error(`Failed to delete profile: ${error.serverError || 'Unknown error'}`, { id: toastRef.current });
    },
    onSettled: () => {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Are you sure you want to delete the profile for {authorName}? This action cannot be undone.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
          <Button
          ...
          >
            {deleteProfileMutation.status === 'executing' ? 'Deleting...' : 'Delete'}
          </Button>
