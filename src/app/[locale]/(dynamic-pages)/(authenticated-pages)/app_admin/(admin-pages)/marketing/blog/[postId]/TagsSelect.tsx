...
export const TagsSelect: React.FC<TagsSelectProps> = ({ post, tags }) => {
  ...
  const updateTagsMutation = useAction(updateBlogPostTagsAction, {
    onExecute: () => {
      toastRef.current = toast.loading('Updating tags...', { description: 'Please wait while we update the tags.' });
    },
    onSuccess: () => {
      toast.success('Done!', {
    ...
    onError: ({ error }) => {
      toast.error(`Failed to update tags: ${error.serverError || 'Unknown error'}`, {
  ...
  const selectedTagIds = post.marketing_blog_post_tags_relationship?.map(t => t.tag_id) || [];
  const options = tags.map(tag => ({ value: tag.id, label: tag.name }));
  const defaultValue = options.filter(option => selectedTagIds.includes(option.value));
