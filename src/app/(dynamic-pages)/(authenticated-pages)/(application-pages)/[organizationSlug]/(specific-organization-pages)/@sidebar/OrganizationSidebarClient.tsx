...
interface OrganizationSidebarClientProps {
...
export default function OrganizationSidebarClient({
  organizationId,
  organizationSlug,
  slimOrganizations,
  subscription
}: OrganizationSidebarClientProps) {
  return (
    <motion.div
      className={cn(
        ...
        >
          {sidebarLinks.map((link) => (
            <motion.div key={link.label} variants={itemVariants}>
              <SidebarLink
                label={link.label}
                href={`/${organizationSlug}${link.href}`}
                icon={link.icon}
              />
            </motion.div>
          ))}
