...
export function handleSupabaseAuthPasswordSignUpErrors(error: AuthError): AuthFormErrorReturnType {
  console.log('--------------------------------')
...
export function handleFlattenedValidationErrors(flattenedErrors: {
  ...
  if (flattenedErrors.fieldErrors) {
    ...
    if (keys.length === 1) {
      ...
      const field = keys.find((key) => flattenedErrors.fieldErrors?.[key]?.length);
      if (field) {
...
function handleTypedValidationErrors<T extends ZodSchema>(
  ...
  if (flattenedErrors.fieldErrors) {
    ...
    const allErrors = keys.flatMap(key => flattenedErrors.fieldErrors![key] || []);
...
export function getSafeActionErrorMessage<T extends ZodSchema>(
  ...
  return fallbackMessage || 'An unexpected error occurred. Please try again or contact support.';
}
