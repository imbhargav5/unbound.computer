...
import { createMarketingChangelogActionSchema, deleteMarketingChangelogSchema, updateChangelogAuthorsSchema, updateMarketingChangelogActionSchema } from '@/utils/zod-schemas/marketingChangelog';
...
export const updateChangelogAuthorsAction = adminActionClient
  ...
  .action(async ({ parsedInput: { changelogId, authorIds } }) => {
    ...
    const authorRelations = authorIds.map(authorId => ({ changelog_id: changelogId, author_id: authorId }));
